...
X_train=['N', 'P', 'K', 'pH', 'rainfall', 'temperature', 'Area_in_hectares',
        ...
        'Crop_Type_rabi', 'Crop_Type_summer', 'Crop_Type_whole_year']
...
def initialize(state,crop,season,area,n,p,k,ph,rain,temp):
    input_data={
        ...
        'Crop_':crop.lower(),
        'Crop_Type_':season.lower().replace(' ','_'),
        'N':float(n),
        'P':float(p),
        'K':float(k),
        'pH':float(ph),
...
def predict():
    if(request.method=='POST'):
        ...
        df_values=initialize(data['state'],data['crop'],data['season'],data['area'],data['nitrogen'],data['phosphorus'],data['potassium'],data['ph'],data['rainfall'],data['temperature'])
